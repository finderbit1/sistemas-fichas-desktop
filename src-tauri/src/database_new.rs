...
impl DatabaseManager {
    ...
    pub async fn run_migrations(&self) -> Result<()> {
        ...
        match &self.config.database_type {
            DatabaseType::Postgres => {
                ...
                // Para PostgreSQL, criar tabelas via SeaORM
    ...
    async fn create_postgres_tables(&self) -> Result<()> {
        // Para PostgreSQL, usar CREATE TABLE IF NOT EXISTS
        ...
        info!("✅ Tabelas PostgreSQL criadas com sucesso");
        Ok(())
    }

    async fn create_sqlite_tables(&self) -> Result<()> {
        // Criar todas as tabelas usando SQLite
        let create_tables = vec![
            // Clientes
            "CREATE TABLE IF NOT EXISTS clientes (
